...
class TimeFold:
    name: str
    train_max_t: int     # включительно
    val_t: int           # один месяц
...
def default_folds(df: pd.DataFrame, time_col: str = "t",
    ...
    """val_months — список (year, month). Если None, используем дефолт."""
    if val_months is None:
        val_months = [(2025, 2), (2025, 1), (2024, 12), (2024, 3)]

    base_year = int(df["year"].min())
    folds = []
    for y, m in val_months:
        val_t = (y - base_year) * 12 + (m - 1)
        # есть ли в данных эта точка?
...
def predict_split(df: pd.DataFrame, time_col: str = "t",
                  predict_year: int = 2025, predict_month: int = 3) -> tuple[np.ndarray, np.ndarray]:
    """Возвращает train_idx (все < predict_t) и predict_idx (predict_t)."""
    base_year = int(df["year"].min())
